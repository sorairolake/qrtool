...
This command encodes input data in a {qrcode-url}[QR code]. By default, the
...
*--foreground* _COLOR_::
  ...
  Foreground color. _COLOR_ takes a {css-color-url}[CSS color string]. Colored
  output is only available when the output format is PNG, SVG or any ANSI
  escape sequences. Note that lossy conversion may be performed depending on
  the color depth supported by the output format. Default is black.
