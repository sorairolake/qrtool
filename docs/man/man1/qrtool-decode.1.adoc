...
vector image. The result will be output to standard output.
...
CAUTION: This command only supports decoding a
{qr-code-model-2-url}[QR code model 2], and does not support decoding a
{micro-qr-code-url}[Micro QR code] and a {rmqr-code-url}[rMQR code].
