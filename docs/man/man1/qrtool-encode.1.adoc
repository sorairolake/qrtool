...
*--zopfli* [_ITERATION_]::
  ...
  Use Zopfli to compress PNG image. Performs compression for the number of
  iterations specified by the value. If the value is not specified, it is
  assumed that 15 is specified. This option requires *--optimize-png*.
endif::[]
