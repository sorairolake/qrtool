...
:qr-code-model-2-url: {qrcode-codes}/model12.html
...
PNG image, a SVG image, a string, or other formats. By default, the result will
...
|Lab function
|`lab(42.5% -21 -20.7)`, `lab(55.8% -3.4 -11.3 / 0.5)`

|LCH function
|`lch(45.5% 69 3.1)`, `lch(55.8% 11.8 253 / 0.5)`
...
In addition to a {qr-code-model-2-url}[QR code model 2], this command can also
...
*-v*, *--symbol-version* _NUMBER_ [_NUMBER_]::
  ...
  version required to store the data will be automatically chosen. For QR code
  model 2, _NUMBER_ should be between *1* and *40*. For Micro QR code, _NUMBER_
...
*-m*, *--margin* _NUMBER_::
  ...
  for QR code model 2 and 2 for others.
...
*--variant* _TYPE_::
  ...
  The possible values are:{blank}:::
    ...
    *normal*::::
      ...
      QR code model 2. This is the default value.
...
Encode in a QR code and output as a SVG image:{blank}::

  $ *qrtool encode -t svg "QR code" > output.svg*

Encode in a Micro QR code:{blank}::

  $ *qrtool encode --variant micro "QR code" > output.png*

Encode in a QR code with the specified colors:{blank}::
